    ...
    def response_inform(self, system_action):
        ...
        if 'taskcomplete' in system_action['inform_slots'].keys(): # check all the constraints from agents with user goal
            ...
            for slot in system_action['inform_slots'].keys():
                ...
                if slot in self.goal['inform_slots'].keys(): #if agent inform slot key is in my inform slots
                    if system_action['inform_slots'][slot] == self.goal['inform_slots'][slot]: #if system value matches my inform value
                        if slot in self.state['rest_slots']: self.state['rest_slots'].remove(slot) # remove from rest slots

                        if len(self.state['request_slots']) > 0: # if there are request slots left
                            self.state['diaact'] = "request"
